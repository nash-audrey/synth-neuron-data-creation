...
beta_parameters = np.zeros(shape=(10,2))
beta_parameters[0,:] = [1,1] # need this even though we draw nothing
beta_parameters[1,:] = [1,1]
beta_parameters[2,:] = [4.9411, 4.9410]
beta_parameters[3,:] = [2.3383, 7.0152]
beta_parameters[4,:] = [7.0098, 2.3360]
beta_parameters[5,:] = [2.9748, 22.7435]
beta_parameters[6,:] = [16.6826, 38.7860]
beta_parameters[7,:] = [33.38976, 33.38971]
beta_parameters[8,:] = [38.8160, 16.6951]
beta_parameters[9,:] = [16.8478, 1.9845]
...
def phase_template_matrix_generator(phase, nTastes):
    ...
    if isinstance(phase, int):
        ...
        elif phase not in range(1,10):
            raise NameError('If you are only giving one number for the Phase Code, it must be 1-9. See the Phase Templates.')
    ...
    if isinstance(phase, list):
        ...
        for i in range(len(phase)):
            if phase[i] not in range(10):
                raise NameError('Acceptable Phase Codes are between 0 and 9. See the Phase Templates.')
    ...
    if type(phase) == np.ndarray:
        ...
        for taste in range(nTastes):
            ...
            for i in range(nLickInts):
                if phase[taste, i] not in range(10):
                    raise NameError('Acceptable Phase Codes are between 0 and 9. See the Phase Templates.')
